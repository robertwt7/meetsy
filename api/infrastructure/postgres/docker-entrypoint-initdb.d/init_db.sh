...
# EOSQL
#
### default database and user for meetsy ##############################################

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
	CREATE USER $MEETSY_POSTGRES_USER WITH PASSWORD '$MEETSY_POSTGRES_PASSWORD';
	CREATE DATABASE $MEETSY_POSTGRES_DB;
	GRANT ALL PRIVILEGES ON DATABASE $MEETSY_POSTGRES_DB TO $MEETSY_POSTGRES_USER;
	ALTER ROLE $MEETSY_POSTGRES_USER CREATEROLE SUPERUSER;
EOSQL
echo
